...
[<UnmanagedFunctionPointer(CallingConvention.Cdecl)>]
type ub_close_cb  = delegate of IntPtr -> unit
...
[<DllImport("libuv.dll", CallingConvention = CallingConvention.Cdecl)>]
extern IntPtr uv_loop_size()

[<DllImport("libuv.dll", CallingConvention = CallingConvention.Cdecl)>]
extern void uv_run(IntPtr loop, int mode)
...
[<DllImport("libuv.dll", CallingConvention = CallingConvention.Cdecl)>]
extern void uv_close(IntPtr handle, ub_close_cb cb)
...
let end_read ok error (client : IntPtr) (nread : int) (buff : byref<uv_buf_t>) = 
  if (nread < 0) then
    if (nread <> UV_EOF) then
      let err = new string(uv_err_name(nread))
      printfn "Read error: %s" err
...
let alloc_buffer (buf : ArraySegment<byte>) (handle : IntPtr) (suggested_size: int) (buff : byref<uv_buf_t>) =
  buff <- uv_buf_t( ``base`` = Marshal.UnsafeAddrOfPinnedArrayElement(buf.Array,buf.Offset), len = buf.Count) 

let receive cn (buf : ByteSegment) =
  Async.FromContinuations <| fun (ok, error, _) ->
    let r = uv_read_start(cn, uv_alloc_cb(fun a b c -> alloc_buffer buf a b &c), uv_read_cb(fun a b c -> end_read ok error a b &c))
    ()

let send cn (buf : ByteSegment) =
  Async.FromContinuations <| fun (ok, error, _) ->
    
    let end_write (req : IntPtr) (status : int) =
      Marshal.FreeHGlobal(req)
      if (status < 0) then 
        let err = new string(uv_strerror(status))
        printfn "Write error: %s" err
        error (Exception err)
      else
        ok <| Choice1Of2 ()

    let req = Marshal.AllocHGlobal(uv_req_size(uv_request_type.UV_WRITE))
    let wrbuff = uv_buf_t( ``base`` = Marshal.UnsafeAddrOfPinnedArrayElement(buf.Array,buf.Offset) , len = buf.Count) 
    let r = uv_write(req, cn, [| wrbuff |], 1, uv_write_cb(end_write))
    ()

type LibUvTransport(client : IntPtr) =
  interface ITransport with
    member this.read (buf : ByteSegment) = receive client buf
    member this.write(buf : ByteSegment) = send client buf