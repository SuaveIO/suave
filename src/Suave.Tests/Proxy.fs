...
[<Tests>]
let proxy =
  let bind :: _ = default_config.properties.bindings
  ...
  let customProperties = { default_config.properties with bindings = [ HttpBinding.mk HTTP IPAddress.Loopback 8084us ] }
  let proxy_config = { default_config with properties = customProperties}
