...
let websocketTests cfg =
  ...
  let websocketApp (webSocket : WebSocket) =
    fun cx -> socket {
      ...
      while !loop do
        ...
        | (Ping, _, _) ->
          do! webSocket.send Pong (Memory<_>([||])) true
        | (Close, _, _) ->
          do! webSocket.send Close (Memory<_>([||])) true
          loop := false
        | _ -> ()
      }
  ...
  let websocketAppReusingBuffers (webSocket : WebSocket) =
    fun cx -> socket {
      let loop = ref true
      while !loop do
        let! msg = webSocket.readIntoByteSegment (fun lengthRequired -> Memory<_>(reuseableBuffer, 0, lengthRequired))
        match msg with
        | (Text, data, true) ->
      ...
      while !loop do
        ...
        | (Text, data, true) ->
          ...
          | "Subprotocol" ->
            ...
            | Some subprotocol ->
              do! webSocket.send Text (Memory<_> (System.Text.Encoding.UTF8.GetBytes subprotocol)) true
            | None ->
              do! webSocket.send Text (Memory<_> [||]) true
          | _ ->
            do! webSocket.send Text data true
        | (Ping, _, _) ->
          do! webSocket.send Pong (Memory<_>([||])) true
